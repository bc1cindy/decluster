"""Reproduce the CTP equal-output consolidation failure mode."""
...
from ..failure_modes import equal_output_consolidation as failure_mode
from ..result_artifacts import canonical_json_bytes, write_canonical_json


EXPERIMENT_ID = "equal-output-consolidation-v1"
...
def _outcome_name(outcome):
    ...
    raise TypeError(f"unsupported consolidation outcome: {type(outcome).__name__}")
...
def build_artifact():
    ...
    return {
        ...
        "scenario": {
            "outputs": [list(output.identifier) for output in scenario.outputs],
            "candidate_sets": [
                sorted(candidate.identifier for candidate in candidates)
                for candidates in scenario.candidate_sets
            ],
        },
        "result": {
...
def verify_artifact(artifact):
    if artifact.get("schema_version") != 1 or artifact.get("experiment") != EXPERIMENT_ID:
        raise VerificationError("unsupported equal-output consolidation artifact identity")
...
def render_markdown(artifact):
    ...
    return "\n".join([
        "# Equal-output consolidation failure mode",
        "",
        "Generated from the canonical experiment artifact. Do not edit manually.",
        "",
        "| quantity | value |",
        "|---|---:|",
        f"| candidate clusters per output before consolidation | {result['candidates_before']} |",
        f"| shared candidate clusters after consolidation | {len(result['candidates_after'])} |",
        f"| surviving cluster | `{result['candidates_after'][0]}` |",
        f"| outcome | `{result['outcome']}` |",
        "",
        "The narrowing is conditional on the later co-spend being evidence of common ownership. "
        "It is not an attribution to a real person.",
        "",
    ])
