...
from ..disjoint_routes import route_capacity
...
def build_artifact(snapshot="data/fs-blkcache-2026-09-04.tar.gz"):
    ...
    capacity, excess, boundary = Counter(), Counter(), Counter()
    measured = 0
...
def render_markdown(artifact):
    ...
    for k, count in sorted(artifact["capacity"].items(), key=lambda kv: int(kv[0])):
    ...
    lines.extend([
        ...
        "",
        "A route is also counted, not weighted. The value it could carry is the plausible-flow half "
        "of the framework's property, and no module here computes it.",
        "",
