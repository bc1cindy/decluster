...
def _seed_arms(ordered, truth, count, rng):
    """Build the real and shuffled seed maps from one stable ranked prefix."""
    chosen = ordered[:count]
    seeds = set(chosen)
    supplied = {vertex: truth[vertex] for vertex in chosen}
    images = list(supplied.values())
    rng.shuffle(images)
    return seeds, supplied, dict(zip(supplied, images))
...
def build_artifact(epochs=DEFAULT_EPOCHS, seed=0):
    ...
    for share in SEED_SHARES:
        ...
        # The control must shuffle a stable list. Iterating ``seeds`` here would make its insertion
        # order depend on Python's per-process hash seed even though the seeded map itself is equal.
        seeds, supplied, shuffled = _seed_arms(ordered, truth, count, rng)
