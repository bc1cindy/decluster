"""Compare agreement-only and Fellegi-Sunter fingerprint ranking."""
...
def evaluate(transactions, weights, label, pair_cap=2000, node_cap=200):
    ...
    ns_auc = fingerprint_ns.pairwise_auc(positives, negatives, axes, weights)
    fs_auc = auc(
    ...
    for index, (query, owner) in enumerate(nodes):
        ...
        if ns_result["best"] is not None and candidates[ns_result["best"]][1] == owner:
            ns_exact += 1
        fs_scores = [scorer.score(query, transaction) for transaction in candidate_transactions]
        fs_best = max(range(len(fs_scores)), key=fs_scores.__getitem__)
        if candidates[fs_best][1] == owner:
            fs_exact += 1
    ...
    return {
        ...
        "ns_auc": ns_auc,
        "fs_auc": fs_auc,
        "ns_top1": ns_exact / evaluated if evaluated else 0.0,
        "fs_top1": fs_exact / evaluated if evaluated else 0.0,
