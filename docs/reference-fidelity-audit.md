...
| Narayanan–Shi–Rubinstein link prediction (2011) | [paper](https://arxiv.org/pdf/1102.4374) | A smaller local adjacency-transfer experiment exists. | Open. The publication uses simulated-annealing seed matching, deterministic/candidate mappings, unanimous voting, and a 25-feature random forest partly trained on de-anonymized test cases. The local common-neighbour experiment is not this algorithm. |
...
| LaurentMT / Boltzmann | [metrics](https://gist.github.com/LaurentMT/e758767ca4038ac40aaf), [LPM](https://gist.github.com/LaurentMT/d361bca6dc52868573a2), [reference code](https://github.com/Samourai-Wallet/boltzmann) | No-fee official vectors A/B/C/P2/P3 match count and LPM. | Open for fees: official P3-with-fees yields 28 weighted combinations; the local unique-mapping model yields 19. Implement Boltzmann traversal multiplicities plus `MERGE_INPUTS`, `MERGE_FEES` and intrafees before claiming parity. |
...
3. Boltzmann parity is partly falsified, not merely unavailable: exact/no-fee vectors agree, while
   the official fee fixture proves that unique set-valued mappings lose tool multiplicity.
...
5. The 2011 cell remains open and should precede any claim-level PAPER/README rewrite. Its actual
