...
| Narayanan–Shi–Rubinstein link prediction (2011) | [paper](https://arxiv.org/pdf/1102.4374) | Algorithm 1's crawl-aware directed cosine, both published threshold policies, Algorithm 3's deterministic/vote/ML cascade, and Algorithm 2's positive-weight distance plus swap/cooling annealer are executable. A smaller local adjacency-transfer experiment remains separately named. | Partial. The pseudocode's ratio is undefined for zero weights even though dummy incident weights are zero; the implementation refuses that case instead of inventing smoothing. Iterative scheduling/revisiting, confidence pruning and the 25-feature random forest remain missing. |
...
5. The 2011 cell is partial: Algorithms 1 and 3, both threshold policies, and Algorithm 2 on its
   ...
   iterative driver and learned-score producer remain open. Its actual composite is materially
   different from `baselines/link_prediction.py`.
