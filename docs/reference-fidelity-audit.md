...
| LaurentMT / Boltzmann | [metrics](https://gist.github.com/LaurentMT/e758767ca4038ac40aaf), [LPM](https://gist.github.com/LaurentMT/d361bca6dc52868573a2), [reference code](https://github.com/Samourai-Wallet/boltzmann) | The dependency-free port reproduces default `LINKABILITY`, fee multiplicity, asymmetric vectors and `MERGE_FEES`. Known-owner input groups are transitively packed, traversed and expanded in the reference order; three official cross-check vectors pin counts and matrices. The older set-valued fee model remains separately named. | `PRECHECK`, `MERGE_OUTPUTS` and JoinMarket intrafees remain absent. They have distinct semantics, not aliases for the default traversal. |
...
3. Boltzmann's default aggregate-traversal multiplicity is now reproduced separately. The
   ...
   parity for output-merge/precheck/intrafee modes remains open.
