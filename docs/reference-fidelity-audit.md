...
| Narayanan–Shi–Rubinstein link prediction (2011) | [paper](https://arxiv.org/pdf/1102.4374) | Algorithm 3's cascade is executable: deterministic mappings take precedence, candidate products vote only when unanimous, and disagreements/missing candidates fall back to an injected ML score. A smaller local adjacency-transfer experiment remains separately named. | Partial. Simulated-annealing seed matching, production of deterministic/candidate mappings, confidence pruning, relaxed stage-2 propagation and the 25-feature random forest remain missing. The local common-neighbour experiment is not this algorithm. |
...
5. The 2011 cell is now partial: its Algorithm 3 combiner is pinned, but the upstream mapping and
   downstream learned-score producers remain open. Its actual composite is materially different
   from `baselines/link_prediction.py`.
