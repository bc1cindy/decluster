...
> **Canonical status (2026-09-04, oracle pinned 2026-09-09).** The deterministic nominal-value
> provenance contract is now generated by `decluster.experiments.ancestry_contract` and registered in
> `catalog/runs/ancestry-contract-v1.json`, which names `ancestry.value_flow_link_oracle` in its own
> artifact instead of inheriting the library default. It covers the absorbing distribution, the exact
> collapsed-to-paper step conversion, typed complete/truncated outcomes, separate truncation causes
> and model-relative provenance overlap. All DSS, mempool.space, WP4, cache, AUC and contiguous-slice
