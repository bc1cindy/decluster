...
Not yet filed against `results/REPRODUCIBILITY.md`, because none of the five states fits. The page
already declares it: the `.blkcache` these figures were measured on was not preserved, so they can
be neither re-asserted (state 1) nor regenerated by a script (state 2). The policy has no state for
a measurement whose data is gone; this document is the case that shows it needs one.
