...
band," not "the schedule identifies the owner." This joins the ~1-day slice (AUC 0.39,
`data/amount-channel-812695-812831-v1.json`)
...
State 2 in `results/REPRODUCIBILITY.md`: the mechanism is pinned; the reported numbers come from a data-run over the `bigquery/temporal.sql` export, which is not committed, and are not asserted.
