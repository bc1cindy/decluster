...
> **Canonical status (2026-09-09).** The deterministic intersection contract is generated by
> `decluster.experiments.intersection_fixture` and registered in
> `catalog/runs/intersection-cluster-fixture-v1.json`, which now names
> `ancestry.value_flow_link_oracle` in its own artifact instead of inheriting the library default.
> Every mainnet number below is the subset-sum walk and stays historical: the seeds are pinned in
> `tests/test_intersection_real.py`, but the fetch protocol and boundary snapshot behind the
> signature sizes were not preserved, so nothing here has been re-measured under value flow.
