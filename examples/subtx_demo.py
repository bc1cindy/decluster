...
from decluster.subtransaction import subtransactions, partition_signal, norm
...
if __name__ == "__main__":
    ...
    print(f"merged transaction {MERGE[:10]}..  ambiguity = {amb} bits ({len(ranked)} balanced partitions)")
    for payment, score, ri, ro in ranked:
        print(f"  payment={payment:>6}  roundness={score}  receiver=in{ri}->out{ro}")
    sig = partition_signal(tx)
    print("\nmost likely:")
    print("  REFUSE (different owners):", sig["refuse"])
    print("  LINK   (input->output):   ", sig["link"])
    print(f"  implicit payment = {sig['payment']}")
